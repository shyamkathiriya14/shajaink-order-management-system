    ...
    <div className="page-entry" style={{ padding: "140px 40px 40px", maxWidth: "1200px", margin: "0 auto" }}>
      <div style={{ marginBottom: "60px", textAlign: "center" }}>
        <h1 style={{ fontSize: "3.5rem", fontWeight: 900, color: "#fff", marginBottom: "16px", letterSpacing: "-0.04em" }}>
          Pending <span style={{ color: "var(--accent)", textShadow: "0 0 30px var(--accent-glow)" }}>Jobs</span>
        </h1>
        <p style={{ color: "var(--text-muted)", fontSize: "1.2rem", fontWeight: 500 }}>Orders waiting for confirmation and scheduling</p>
      </div>

      <div style={{ display: "grid", gap: "20px" }}>
        {jobs.length === 0 ? (
          <div className="glass-card" style={{ padding: "120px", textAlign: "center", border: "1.5px dashed var(--border)" }}>
            <p style={{ fontSize: "1.6rem", color: "var(--text-muted)", fontWeight: 600 }}>No pending jobs found</p>
          </div>
        ) : (
          jobs.map(job => (
            <JobCard key={job.id} job={job} onDelete={handleDelete}>
              <button 
                ...
                style={{ 
                  ...
                  boxShadow: "0 0 20px var(--warning-glow)",
                  color: "#000"
                }}
              >
                <span>📅</span> MOVE TO UPCOMING
              </button>
            </JobCard>
          ))
        )}
